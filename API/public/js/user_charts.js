...
async function fetchChartData() {
    try {
        // You should replace ':id' with the actual player ID.
        const playerId = 'somePlayerId'; // This needs to be dynamic based on your app's logic
        const levels_response = await fetch(`http://localhost:5000/api/players/${playerId}/summary`, {method: 'GET'});

        if(levels_response.ok) {
            const results = await levels_response.json();
            console.log(results);
 
// Call fetchChartData when the DOM is fully loaded
